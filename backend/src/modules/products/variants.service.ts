...
export class VariantsService {
  ...
  private async createVariant(productId: string, dto: CreateVariantDto): Promise<ProductVariant> {
    ...
    try {
      saved = await this.variantRepo.save(variant);
    } catch (err) {
      if (err instanceof QueryFailedError && (err as QueryFailedError & { code?: string }).code === '23505') {
    ...
    // Create inventory row for each store (0 quantity) so variant shows in New sale dropdown
    try {
      ...
      this.logger.error('Failed to create inventory rows for variant', err);
      if (err instanceof QueryFailedError && (err as QueryFailedError & { code?: string }).code === '23505') {
        throw new ConflictException('Variant was created but inventory setup failed (duplicate). Refresh the variant list.');
      }
      throw err;
    }
    return saved;
